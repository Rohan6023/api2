...
def configure_ngrok():
    ngrok_token = "2mKpcWFiiEXzkzj2K7DBozb8IK8_34VLmjzUVEPSffmioFmXB"
    try:
...
def update_soul_txt(public_url):
    with open("ninjagamerop1.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in ninjagamerop1.txt")

def update_vps_soul_txt(public_url):
    vps_ip = "194.238.17.134"
    vps_user = "ubuntu"
    vps_password = "soulKing901210099090"
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("ninjagamerop1.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated ninjagamerop1.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update ninjagamerop1.txt on VPS: {str(e)}")
